...
class TeacherController(private val adminService: AdminService) {

    @GetMapping("/topics")
    fun listTopics(): List<AdminTopicDto> = adminService.listTopicsForManagement()

    @PutMapping("/topics/{id}/publish")
    fun setPublished(@PathVariable id: Long, @RequestBody request: PublishUpdateRequest): AdminTopicDto =
        adminService.setTopicPublished(id, request.published)
}
